...
def load_all_models():
    ...
    try:
        ...
        # Load MLP
        if not os.path.exists(os.path.join(save_path, 'mlp_model.pkl')):
            st.error("MLP model file not found at " + os.path.join(save_path, 'mlp_model.pkl') + ". Please ensure it's saved.")
            # Fallback for demonstration if model file is truly missing
            from sklearn.neural_network import MLPClassifier
            dummy_model_mlp = MLPClassifier(hidden_layer_sizes=(100,), max_iter=300, random_state=42)
            dummy_model_mlp.fit(np.random.rand(10, len(selected_features)), np.random.randint(0, 2, 10))
            models['MLP'] = dummy_model_mlp
        else:
            models['MLP'] = joblib.load(os.path.join(save_path, 'mlp_model.pkl'))

    except Exception as e:
        st.error(f"Error loading or initializing models: {e}")
        ...
        from sklearn.neural_network import MLPClassifier
        models['MLP'] = MLPClassifier(hidden_layer_sizes=(100,), max_iter=300, random_state=42)
        models['MLP'].fit(np.random.rand(10, len(selected_features)), np.random.randint(0, 2, 10))
    return models
...
if uploaded_file is not None:
    try:
        ...
        # --- Plotting Uploaded Data Distributions ---
        st.subheader("Uploaded Data Feature Distributions:")
        for col in batch_df.columns:
            if col in selected_features:
                fig, ax = plt.subplots(figsize=(8, 4))
                if pd.api.types.is_numeric_dtype(batch_df[col]):
                    sns.histplot(batch_df[col], kde=True, ax=ax)
                    ax.set_title(f'Distribution of {col.replace("_", " ").title()}')
                    ax.set_xlabel(col.replace("_", " ").title())
                    ax.set_ylabel('Count / Density')
                else:
                    # For categorical features, use countplot
                    sns.countplot(y=batch_df[col], order=batch_df[col].value_counts().index, ax=ax)
                    ax.set_title(f'Distribution of {col.replace("_", " ").title()}')
                    ax.set_xlabel('Count')
                    ax.set_ylabel(col.replace("_", " ").title())
                st.pyplot(fig)
                plt.close(fig) # Close the figure to free memory
        # --- End Plotting Uploaded Data Distributions ---
        ...
        if st.button("Run Batch Prediction"):
            ...
            if missing_cols:
                ...
                # --- Plotting Predicted Data Distributions ---
                st.subheader("Predicted Results Visualizations:")

                # Plot 1: Distribution of Predicted Resistance Classes
                fig1, ax1 = plt.subplots(figsize=(8, 5))
                sns.countplot(x='Predicted_Resistance', data=batch_df, palette='viridis', ax=ax1)
                ax1.set_title('Distribution of Predicted Resistance Classes')
                ax1.set_xlabel('Predicted Class')
                ax1.set_ylabel('Number of Samples')
                st.pyplot(fig1)
                plt.close(fig1)

                # Plot 2: Distribution of Resistance Probability
                fig2, ax2 = plt.subplots(figsize=(8, 5))
                sns.histplot(batch_df['Resistance_Probability'], kde=True, ax=ax2, bins=20)
                ax2.set_title('Distribution of Predicted Resistance Probability')
                ax2.set_xlabel('Resistance Probability (%)')
                ax2.set_ylabel('Count / Density')
                st.pyplot(fig2)
                plt.close(fig2)
                # --- End Plotting Predicted Data Distributions ---
